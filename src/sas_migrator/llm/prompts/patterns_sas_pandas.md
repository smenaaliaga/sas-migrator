...
| PROC HTTP a la API BDE del Banco Central (`si3.bcentral.cl/SieteRestWS`) | `r = requests.get("https://si3.bcentral.cl/SieteRestWS/SieteRestWS.ashx", params={"user": os.environ["BDE_USER"], "pass": os.environ["BDE_PASS"], "function": "GetSeries", "timeseries": serie, "firstdate": desde, "lastdate": hasta}, timeout=30)` → `pd.json_normalize(r.json()["Series"]["Obs"])` — campos `indexDateString` (dd-MM-yyyy) y `value` (string; `pd.to_numeric(..., errors="coerce")`) |
| Host con `mode=sdk` en «Conexiones externas» del contexto | usa SOLO el paquete declarado (ej. `bcchapi` para la API BDE) — jamás mezclar SDK con requests al mismo host, jamás replicar la URL cruda |
